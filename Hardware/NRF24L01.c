...
/*依次存放各按键状态的结构体数组
tx_buffer[0]---WKUP 按下为1，其余为0
tx_buffer[1]---KEY_1
tx_buffer[2]---SW_shang
tx_buffer[3]---SW_xia
tx_buffer[4]---SW_zuo
tx_buffer[5]---SW_you
tx_buffer[6]---SW_UP
tx_buffer[7]---SW_DOWN
tx_buffer[8]---SW_LEFT
tx_buffer[9]---SW_RIGHT
tx_buffer[10]---Rocker_Right_X_Low_Half
tx_buffer[11]---Rocker_Right_X_High_Half
tx_buffer[12]---Rocker_Right_Y_Low_Half
tx_buffer[13]---Rocker_Right_Y_High_Half
tx_buffer[14]---Rocker_Left_X_Low_Half
tx_buffer[15]---Rocker_Left_X_High_Half
tx_buffer[16]---Rocker_Left_Y_Low_Half
tx_buffer[17]---Rocker_Left_Y_High_Half
tx_buffer[18]---Rocker_Right_SW
tx_buffer[19]---Rocker_Left_SW
*/
...
{
	memcpy(tx_buffer,key_event_buffer,10);  //把按键相关数据加载到tx_buffer
	tx_buffer[10]=AD_Value[Rocker_Right_X]&0xFF; //Rocker_Right_X_Low_Half
	tx_buffer[11]=AD_Value[Rocker_Right_X]>>8; 	//Rocker_Right_X_High_Half
	tx_buffer[12]=AD_Value[Rocker_Right_Y]&0xFF; //Rocker_Right_Y_Low_Half
	tx_buffer[13]=AD_Value[Rocker_Right_Y]>>8; 	//Rocker_Right_Y_High_Half
	tx_buffer[14]=AD_Value[Rocker_Left_X]&0xFF; //Rocker_Left_X_Low_Half
	tx_buffer[15]=AD_Value[Rocker_Left_X]>>8; 	//Rocker_Left_X_High_Half
	tx_buffer[16]=AD_Value[Rocker_Left_Y]&0xFF; //Rocker_Left_Y_Low_Half
	tx_buffer[17]=AD_Value[Rocker_Left_Y]>>8; 	//Rocker_Left_Y_High_Half
...
uint8_t NRF24L01_RxPacket(uint8_t *rxbuf)
{
    ...
    if(status & (1<<RX_DR)) // 如果接收到数据 (RX_DR 置位)
    {
        ...
        if(!(fifo_status & (1<<6))) // 如果RX FIFO不为空 (RX_EMPTY位为0)
        {
            NRF24L01_Read_Buf(RD_RX_PLOAD, rxbuf, NRF_PAYLOAD_LENGTH); // 读取接收到的数据
            // 不需要在这里清除RX_DR，因为在中断处理函数中会清除
            return 1; // 接收成功
        }
    }
    return 0; // 未接收到数据
